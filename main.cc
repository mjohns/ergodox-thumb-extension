...
int main() {
  ...
  Shape base_inner_right =
      Cube(short_width - kCornerRadius * 2, long_width - kCornerRadius * 2, 1)
          .Projection()
          .LinearExtrude(1);
  // Cut a little of the top left corner to accomadate where the thumb keys meet
  // the other keys.
  Shape base_inner_bottom = base_inner_right.RotateZ(90).Subtract(
      Cube(10, 7, 1).RotateZ(45).Translate(long_width / (-2), short_width / 2,
                                           0));

  base_inner_right = base_inner_right.Translate(long_width - short_width / 2,
                                                long_width / 2, 0);
  base_inner_bottom =
      base_inner_bottom.Translate(long_width / 2, short_width / 2, 0);

  Shape base_right =
      Minkowski(base_inner_right, corner).Projection().LinearExtrude(1);
  Shape base_bottom =
      Minkowski(base_inner_bottom, corner).Projection().LinearExtrude(1);

  Shape base_outline_right = base_right - base_inner_right;
  Shape base_outline_bottom = base_bottom - base_inner_bottom;

  Shape base_outline = (base_outline_right + base_outline_bottom) -
                       (base_inner_bottom + base_inner_right);

  // Shapes to cut any overhang off the sides and top.
  Shape full_base = Union(base_right, base_bottom);
  Shape horizontal_bounds =
      Cube(1000, 1000, 1000)
          .Subtract(full_base.Projection().LinearExtrude(1000));

  Shape final_key_holes =
      UnionAll(key_holes).RotateY(kRotationY).TranslateZ(kPlateHeight);
  Shape top_plate = Cube(500, 500, kSwitchThickness)
                        .TranslateZ(kSwitchThickness / (-2))
                        .Add(UnionAll(switches))
                        .RotateY(kRotationY)
                        .TranslateZ(kPlateHeight);
  Shape top_bounds = Cube(800, 800, 100)
                         .TranslateZ(50)
                         .RotateY(kRotationY)
                         .TranslateZ(kPlateHeight);
  ...
  ext.WriteToFile("left.scad");
  ext.MirrorX().WriteToFile("right.scad");
}
